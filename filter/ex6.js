...
function filterOffensiveComments(comments, bannedWords) {
  // comments.filter(comment => comment.toLowelCase().includes(
  //   bannedWords.filter(bannedWord => bannedWord.toLowelCase())
  // ))
  res = comments;
  for (c = 0; c < bannedWords.length; c++) {
    res = res.filter(comment => !comment.toLowerCase().includes(bannedWords[c]));  
  }
  return res;
}
